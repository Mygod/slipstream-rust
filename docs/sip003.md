...
`keep-alive-interval`, and `quic-idle-timeout-seconds`. Server consumes `domain`, `cert`, `key`,
`reset-seed`, `fallback`, `max-connections`, and `keep-alive-interval`.
...
`keep-alive-interval` is in milliseconds for both client and server (for example, `keep-alive-interval=400`).
`quic-idle-timeout-seconds` controls picoquic's transport idle timer.
