use crate::picoquic::{
    ...
    picoquic_set_stream_data_consumption_mode, slipstream_take_stateless_packet,
...
pub unsafe fn take_stateless_packet(quic: *mut picoquic_quic_t) -> Option<Vec<u8>> {
    if quic.is_null() {
        return None;
    }
    ...
    let ret =
        slipstream_take_stateless_packet(quic, buffer.as_mut_ptr(), buffer.len(), &mut length);
    if ret <= 0 {
        return None;
    }
